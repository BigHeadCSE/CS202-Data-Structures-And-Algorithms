...
class RBTree {
	...
	int rangeSum(Node *nodeZ, int i, int j){
	    int leftval= nodeZ->left->num;

        if( j-i+1 == nodeZ->num)  return nodeZ->val;
        else if( j<= leftval) return rangeSum(nodeZ->left, i, j);
        else if( i>= leftval) return rangeSum(nodeZ->right, i-leftval, j-leftval);
        else if( i<= leftval && j>=leftval){
